...
int isClear = true; //maybe fix later to be read better.
...
void loop(){
  /******** Checking ADD *************************/
  isClear = digitalRead(BTN_2);
  if (isClear != true) {
    int CHAR_LEN = 4;
    int add[CHAR_LEN] = {KEY_A, KEY_D, KEY_D, KEY_RET};
  ...
  /******** Checking MOVE ************************/
  isClear = digitalRead(BTN_3);
  if (isClear != true) {
    int CHAR_LEN = 5;
    int move[CHAR_LEN] = {KEY_M, KEY_O, KEY_V, KEY_E, KEY_RET};
    for(int i = 0; i < CHAR_LEN; i++){
      buf[2] = move[i];
      Serial.write(buf, 8); // Send keypress
      releaseKey();
  ...
  /******** Checking COPY ************************/
  isClear = digitalRead(BTN_4);
  if (isClear != true) {
    int CHAR_LEN = 5;
    int copy[CHAR_LEN] = {KEY_C, KEY_O, KEY_P, KEY_Y, KEY_RET};
    for(int i = 0; i < CHAR_LEN; i++){
      buf[2] = copy[i];
      Serial.write(buf, 8); // Send keypress
      releaseKey();
  ...
  /******** Checking RATSNEST ************************/
  isClear = digitalRead(BTN_5);
  if (isClear != true) {
    int CHAR_LEN = 5;
    int rats[CHAR_LEN] = {KEY_R, KEY_A, KEY_T, KEY_S, KEY_RET};
    for(int i = 0; i < CHAR_LEN; i++){
      buf[2] = rats[i];
      Serial.write(buf, 8); // Send keypress
      releaseKey();
